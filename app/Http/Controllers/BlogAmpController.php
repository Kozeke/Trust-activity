...
class BlogAmpController extends \App\Modules\Panel\Controllers\AbstractController
{
    /**
    ...
    public function index()
    {
        ...
        $blog       = BlogArticles::orderBy('created_at', 'DESC')->offset($this->offset)->limit($this->per_page)->get();
        $categories = BlogCategories::orderBy('created_at', 'ASC')->get();

        return view('amp.index')->with(['articles' => $blog, 'categories' => $categories, 'pagination' => $this->pagination]);
    }
    ...
    public function category($category)
    {
        ...
        if ($current_category)
        {
            ...
            $blog      = BlogArticles::where('category_id', $current_category->id)->offset($this->offset)->limit($this->per_page)->get();
            $categories = BlogCategories::orderBy('created_at', 'ASC')->get();
 
            return view('amp.category')->with(['articles' => $blog, 'categories' => $categories, 'pagination' => $this->pagination]);
    ...
    public function show($category, $url)
    {
        ...
        return view('amp.item')->with(['article' => $blog, 'categories' => $categories]);
    }
