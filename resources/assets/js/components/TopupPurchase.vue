<template>
    ...
    <div class="active_domain_block">
        <div class="active_domain_fon" v-bind:class="{ show_topup_plan: show_topup_plan }"></div>
        <div class="c-active_domain_block" v-bind:class="{ show_topup_plan: show_topup_plan }">
        	<div style="position: relative;">
	        	<div class="domain_modal_preloader"></div>
	            <!--<div class="active_domain_name">mail@instanobel.com</div>-->
	            <div class="topup-step" v-bind:class="{ active: TopupPlan.step == '1' }">
	                <div class="active_domain_title">Top Up Your Balance</div>
	                <div class="active_domain_plan">
	                    <p class="active_domain_plan_text">Select payment method:</p>
	                    <div class="payment_select">
	                        <div class="active_domain_payment_item" @click="selectMethod('paypal')" v-bind:class="{ active: TopupPlan.pay_type == 'paypal' }">
	                            <p class="payment_paypal"></p>
	                        </div>
	    					<div class="active_domain_payment_item" @click="selectMethod('checkout')" v-bind:class="{ active: TopupPlan.pay_type == 'checkout' }">
	                            <p class="payment_checkout"></p>
	                        </div> 
	                    </div>
	                    <transition name="fade">
	                        <div class="active_domain_link" v-show="TopupPlan.pay_type != null">
	            ...
	            <div class="topup-step step-2" v-bind:class="{ active: TopupPlan.step == '2' }">
        ...
        directives: {
	 	...
	 	methods: {
            selectMethod: function(type) {
                this.TopupPlan.pay_type = type;
            },
            SendPayment: function() {

                if (this.TopupPlan.pay_type == 'checkout') {
                    this.TopupPlan.step = 2;
                } else if (this.TopupPlan.pay_type == 'paypal') {
	                $('.show_topup_plan .domain_modal_preloader').fadeIn('fast');
        			$( "#payment-form" ).submit();	
                }
            },
				...
				if ($('#checkout-phone').val().length < 1) { $('#checkout-phone').addClass('error'); errors++; $('#checkout-phone-error').css('display','block')} else { $('#checkout-phone').removeClass('error');$('#checkout-phone-error').css('display','none')  }
 				...
 				if(errors == 0) {
					var data = {    
						...
						'payment_amount' : $('input[name="payment-amount"]').val(),
						'payment_type' : $('input[name="payment-type"]').val(),
					}; 		
